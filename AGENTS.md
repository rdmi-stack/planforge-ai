...
- **ODM**: Beanie 1.27+ with the async PyMongo driver
- **Database**: MongoDB Atlas (primary), Redis 7 (caching, queues, sessions)
...
│   │   ├── database.py                # Async PyMongo client + Beanie initialization
...
│   │
│   ├── migrations/                    # Reserved for future data migration scripts
│   │   ├── env.py
│   │   └── versions/
│   │
│   └── tests/
...
async def create_project(
    ...
    project = await ProjectService().create(user_id=user.id, data=data)
    return project
```
```python
...
- Never build MongoDB queries from untrusted string interpolation. Validate inputs first and keep filters explicit.
...
- Database: reuse the shared async PyMongo client and keep Atlas connection counts under control.
...
- Test database: use a dedicated MongoDB Atlas test database or local Mongo instance
...
# Backend (.env)
MONGODB_URI=mongodb+srv://<username>:<password>@<cluster-url>/planforge
MONGODB_DB_NAME=planforge
...
5. **MongoDB Atlas is the source of truth**: use Beanie documents and the async PyMongo client for persistence.
